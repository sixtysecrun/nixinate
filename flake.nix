          ...
          let
            ...
            mkDeployScript = { machine, dryRun }: let
              ...
              '' + (if hermetic then ''
                ...
                ( set -x; ${openssh} -t ${user}@${host} "doas nix-store --realise ${nixos-rebuild} ${flock} && doas ${flock} -w 60 /dev/shm/nixinate-${machine} ${nixos-rebuild} ${nixOptions} ${switch} --flake ${flake}#${machine}" )
              '' else ''
                echo "🤞 Activating configuration non-hermetically on ${machine} via ssh:"
                ( set -x; ${openssh} -t ${user}@${host} "doas flock -w 60 /dev/shm/nixinate-${machine} nixos-rebuild ${switch} --flake ${flake}#${machine}" )
              '')
              else ''
